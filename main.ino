...
bool APreconnet = 0;
bool flag_ctrbuzz = 1;
bool flag_ctrbuzz_blynk = 1;
bool auto_mode = 1;
bool relay_status = 0;
bool relay_status_blynk = 0;
bool flag_scan = 0;
...
void read_Serial(void) {
  if (Serial.available()) {
    ...
    if (data.indexOf("K-") != -1) {
      ...
      int first = 4;
      int last = data.indexOf("-", first);
      buff = data.substring(first, last);
      first = last + 1;
      last = data.indexOf("-", first);
      int V20 = data.substring(first, last).toInt();
      first = last + 1;
      last = data.indexOf("-", data.length());
      int V21 = data.substring(first, last).toInt();
      if (V21) flag_ctrbuzz_blynk = 1;
      else flag_ctrbuzz_blynk = 0;
      if (V20) relay_status_blynk = 1;
      else relay_status_blynk = 0;
      if (flag_connected) {
...
void auto_Control(void) {
  ...
  if (!flag_ctrbuzz || dustVal < alert)
    analogWrite(buzzer, 0);
  if (!relay_status_blynk) {
  ...
  if ((flag_waitDeleting == 0) && (flag_waitAdding == 0)) {
    if (flag_menu == 0) {
      if (flag_updateLCD) {
        ...
        if (flag_SSshow == 0) {
          show_LCD("Gateway", 0, 1);
          show_LCD("value: " + String(dustVal), 1, 0);
        } else {
...
void make_Request(void) {
  ...
  if (flag_menu == 1) {
    ...
    if (line == 1) {
      ...
      Serial.print("set Realy status: ");
      Serial.println(relay_status);
      s = "R-" + (String)relay_status;
    }
    if (line == 2) {
      auto_mode = !auto_mode;
      EEPROM.writeByte(299, auto_mode);
...
void delete_someSensor(int index_delete) {
  for (int i = index_delete; i <= num_sensor; ++i)
...
void config_value(void) {
  ...
  auto_mode = EEPROM.readByte(299);
  alert = EEPROM.readFloat(300);
  for (int i = 1; i <= num_sensor; ++i)
    sensor_ID[i - 1] = EEPROM.readByte(i);
}
void readMode(void) {
  if (flag_mode) {
    flag_mode = 0;
